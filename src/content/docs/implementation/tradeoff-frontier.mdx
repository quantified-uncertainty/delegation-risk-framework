  ...
  <script>{`
    (function() {
      ...
      function draw() {
        ...
        // Draw budget line (iso-ETE curve approximation)
        if (budgetLine > 0) {
          ...
          // ETE ≈ capability × (100 - trust) × factor
          // Rearrange: trust = 100 - ETE / (capability × factor)
...
Use when: Stakes are high, trust budget is small
```
...
Use when: High potential value, substantial trust budget
```
...
The orange dashed line (when budget > 0) shows configurations with approximately equal Expected Trust Exposure. Points below this line exceed your risk budget.
...
3. **Set budget line**: What ETE can you accept?
...
- [ETE Calculator](/implementation/ete-calculator/) — Quantify your risk exposure
- [Trust Propagation](/implementation/trust-propagation/) — Model trust through delegation chains
- [Decision Guide](/implementation/decision-guide/) — Choose implementation approach
